...
const source = await readFile(resolve(process.cwd(), "contracts/VinylLog.sol"), "utf8");
const input = {
  language: "Solidity",
  sources: { "VinylLog.sol": { content: source } },
  settings: { outputSelection: { "*": { "*": ["abi", "evm.bytecode.object"] } } },
};
const output = JSON.parse(solc.compile(JSON.stringify(input)));
const contract = output.contracts?.["VinylLog.sol"]?.VinylLog;
...
console.log(`Deploying VinylLog from ${account.address} on Base...`);
...
console.log("Set NEXT_PUBLIC_VINYL_LOG_CONTRACT_ADDRESS to this contract address in Vercel.");
